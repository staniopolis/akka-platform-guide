...
We suggest that you read xref:deployment:index.adoc[] to get an understanding of how deployment with the Akka Cloud Platform works.

Before following the steps below, create a xref:deployment:aws-install.adoc[Kubernetes cluster and install the Akka Operator].

=== Build Docker image

Create a Docker repository and authenticate Docker as described in xref:deployment:aws-ecr.adoc[Amazon Elastic Container Registry].
